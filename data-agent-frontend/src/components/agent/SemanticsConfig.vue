...
<template>
  <div class="semantic-model-config">
    <div class="config-card">
      <div class="card-header">
        <h2 class="section-title">语义模型管理</h2>
        <p class="section-desc">管理数据库表字段的业务语义，帮助 LLM 更准确地理解数据结构。</p>
      </div>

      <!-- Toolbar -->
      <div class="action-bar">
        <div class="left-actions">
          <h3 class="subsection-title">语义模型列表</h3>
          <el-button
        ...
        <div class="right-actions">
          ...
          <el-button @click="openBatchImportDialog" type="success" plain :icon="UploadFilled">
            批量导入
          </el-button>
          <el-button @click="openCreateDialog" type="primary" :icon="Plus">添加语义模型</el-button>
        </div>
      </div>
      ...
      <div class="table-container">
        ...
        >
          ...
          <el-table-column prop="tableName" label="表名" min-width="150">
            <template #default="scope">
              <span class="code-text text-indigo">{{ scope.row.tableName }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="columnName" label="字段名" min-width="150">
            <template #default="scope">
              <span class="code-text text-slate">{{ scope.row.columnName }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="businessName" label="业务名称" min-width="150">
            <template #default="scope">
              <span class="font-medium">{{ scope.row.businessName }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="synonyms" label="同义词" min-width="150" show-overflow-tooltip>
            <template #default="scope">
              <span v-if="scope.row.synonyms" class="text-gray-600">{{ scope.row.synonyms }}</span>
              <span v-else class="text-gray-400 text-xs">-</span>
            </template>
          </el-table-column>
          <el-table-column prop="dataType" label="类型" width="100">
            <template #default="scope">
              <el-tag type="info" size="small" effect="plain" class="mono-tag">
                {{ scope.row.dataType }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column label="状态" width="100" align="center">
            <template #default="scope">
              <el-switch
                ...
                @change="val => toggleStatus(scope.row, val ? 1 : 0)"
                size="small"
          ...
          <el-table-column label="操作" width="150" fixed="right">
            <template #default="scope">
              <div class="operation-buttons">
                <el-button @click="editModel(scope.row)" size="small" type="primary" link>
                  编辑
                </el-button>
                <el-button @click="deleteModel(scope.row)" size="small" type="danger" link>
                  删除
                </el-button>
              </div>
            </template>
    ...
    <el-dialog
      ...
      destroy-on-close
    >
      <el-form :model="modelForm" label-width="100px" ref="modelFormRef" class="model-form">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="表名" prop="tableName" required>
              <el-input v-model="modelForm.tableName" placeholder="请输入数据库表名" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="字段名" prop="columnName" required>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="业务名称" prop="businessName" required>
              <el-input v-model="modelForm.businessName" placeholder="例如：用户ID" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="数据类型" prop="dataType" required>
              <el-input v-model="modelForm.dataType" placeholder="例如：int, varchar(50)" />
            </el-form-item>
          </el-col>
        </el-row>
